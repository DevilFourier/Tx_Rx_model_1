function [] = rx(y_ch,Awgn)
    ...
    %RANDOM SAMPLING
    [block.y_rx_sam,idx] = random_sampling(y_ch,sig.n_sam_sy_ch,sig.n_add_sam_ch,2*sig.n_sy,sig.T_sam_ac/sig.T_sam_ch);
    
    %OVERSAMPLING
    block.t_rx_sam = linspace(block.t_ch(idx),sig.T_sy*(2*sig.n_sy-1)+block.t_ch(idx),sig.n_sam_ac);
    block.t_rx = linspace(block.t_ch(idx),sig.T_sy*(2*sig.n_sy-1)+block.t_ch(idx),sig.n_sam_rx);
    block.y_rx = interp1(block.t_rx_sam,block.y_rx_sam,block.t_rx);
    
    %CLOCK GENERATION
    block.clock_rx = clock_gen(sig.n_sy_c,1+sig.n_add_sam_rx);
